...
//	char attr[256] = {0};
	...
	{
		cgiFormStringNoNewlines("level", level, 32);
	...
	strcat(templateFileName, "log.htm");
		...
		while (1)
		{
			...
			else if (count > 0)
			{
				...
				if (strstr(buf, "<!-- query data -->") != NULL)
				{
					...
					if (queryDatabase(level, startdate, stopdate) < 0)
					{
...
void printWebPageTableTitle()
{
	...
	fprintf(cgiOut, "<td align=\"center\">级别</td>\n");
	fprintf(cgiOut, "<td align=\"center\">内容</td>\n");
	fprintf(cgiOut, "<td align=\"center\">日期</td>\n");
	fprintf(cgiOut, "<td align=\"center\">次数</td>\n");
	fprintf(cgiOut, "</tr>\n");
}
...
//	char errmsg[256];
	...
	{
		strcpy(sql, "select * from log");
	}
	else
	{
		strcpy(sql, "select 级别, 内容, 日期, 次数 from log where ");
	
		memset(strtmp, 0, 128);
		if (strcmp(waynum, "") != 0)
		{
			sprintf(strtmp, "级别 = '%s' ", waynum);
			strcat(sql, strtmp);
		}
		...
		{
			sprintf(strtmp, "and 日期 > '%s' ", startdate);
			strcat(sql, strtmp);
		}
		...
		{
			sprintf(strtmp, "and 日期 <= '%s'", stopdate);
			strcat(sql, strtmp);
		}
	}
